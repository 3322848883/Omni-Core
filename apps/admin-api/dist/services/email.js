...
class EmailService {
    ...
    initializeTransporter() {
        const { smtp } = config_1.config;
        if (!smtp.host || !smtp.user || !smtp.password) {
            logger_1.logger.warn('Email service is not configured. SMTP settings are missing.');
        ...
        try {
            ...
            logger_1.logger.info(`Email service initialized with SMTP host: ${smtp.host}`);
        }
        catch (error) {
    ...
    async sendEmail(options) {
        if (!this.isConfigured || !this.transporter) {
            logger_1.logger.warn('Email service is not configured. Cannot send email.');
            return false;
        }
        try {
            const { smtp } = config_1.config;
            ...
            logger_1.logger.info(`Email sent successfully. MessageId: ${result.messageId}`);
            return true;
        }
        catch (error) {
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="notice">
        ...
        <p>4. 管理后台地址：<a href="${config_1.config.adminWebUrl}/orders">${config_1.config.adminWebUrl}/orders</a></p>
      </div>
    </div>
    <div class="footer">
...
4. 管理后台地址：${config_1.config.adminWebUrl}/orders
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="success-notice">
        ...
        <p>管理后台：<a href="${config_1.config.adminWebUrl}/orders">${config_1.config.adminWebUrl}/orders</a></p>
      </div>
      `}
    </div>
    <div class="footer">
...
管理后台：${config_1.config.adminWebUrl}/orders
`}
