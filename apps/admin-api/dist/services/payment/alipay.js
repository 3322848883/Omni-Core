...
exports.AlipayPaymentProvider = void 0;
...
class AlipayPaymentProvider {
    ...
    receiverName;
    constructor() {
        this.qrCodeUrl = config_1.config.payment?.alipay?.qrCodeUrl || '';
        this.receiverName = config_1.config.payment?.alipay?.receiverName || '';
        if (!this.qrCodeUrl) {
            logger_1.logger.warn('Alipay QR code URL is not configured');
        }
        logger_1.logger.info('Alipay personal QR code payment provider initialized');
    }
    /**
     * 创建支付
     * 返回收款码URL和转账信息
    ...
    async createPayment(request) {
        try {
            logger_1.logger.info(`Creating Alipay payment for order: ${request.orderNo}`);
            // 生成支付说明
            ...
            logger_1.logger.error('Failed to create Alipay payment:', error);
            throw error;
        }
    }
    /**
    ...
    async processRefund(request) {
        logger_1.logger.warn('Alipay personal QR code refund must be processed manually');
        return {
            success: false,
            refundId: '',
...
exports.AlipayPaymentProvider = AlipayPaymentProvider;
