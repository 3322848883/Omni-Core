 ...
 * - 更专业的支付体验
 */
export class WechatMerchantProvider implements IPaymentProvider {
  readonly name: PaymentProvider = 'wechat_merchant';
  ...
  constructor() {
    this.mchId = config.payment?.wechatMerchant?.mchId || '';
    this.appId = config.payment?.wechatMerchant?.appId || '';
    this.apiKey = config.payment?.wechatMerchant?.apiKey || '';
    this.sandbox = config.payment?.wechatMerchant?.sandbox !== false;
  ...
  async createPayment(request: CreatePaymentRequest): Promise<CreatePaymentResponse> {
    try {
      logger.info(`Creating WeChat merchant payment for order: ${request.orderNo}`);
      ...
      const params = {
        ...
        notify_url: config.apiUrl + '/webhooks/wechat',
        trade_type: 'NATIVE',
  ...
  private generateSign(params: Record<string, unknown>): string {
    ...
    logger.debug('Generated WeChat sign:', signature);
    return signature;
  }
   ...
   * 构建 XML 请求体
   */
  private buildXmlBody(params: Record<string, unknown>): string {
  ...
  parseWebhookEvent(rawBody: string, signature: string): PaymentWebhookEvent {
    try {
      ...
      logger.error('Failed to parse WeChat webhook event:', error);
      throw error;
    }
  }
  ...
  async handlePaymentSuccess(event: PaymentWebhookEvent): Promise<{
    ...
    // 验证并提取必要字段
    const transactionId = validateString(data.transaction_id, 'transaction_id') || '';
    ...
    if (isNaN(totalFee) || totalFee < 0) {
      throw new PaymentError('Invalid total_fee', 'INVALID_AMOUNT', this.name);
    }
    ...
    try {
      ...
      logger.warn('Failed to parse attach:', e);
    }
  ...
  async handlePaymentFailure(event: PaymentWebhookEvent): Promise<{
    ...
    // 安全地提取字段
    const transactionId = validateString(data.transaction_id, 'transaction_id', { required: false }) || '';
    const errCodeDes = validateString(data.err_code_des, 'err_code_des', { required: false }) || 'Payment failed';
  ...
  async processRefund(request: RefundRequest): Promise<RefundResponse> {
    try {
      logger.info(`Processing WeChat refund for payment: ${request.paymentId}`);
      ...
      // 实际实现需要调用微信退款接口
      logger.debug('WeChat refund request:', xmlBody);
      ...
      logger.error('Failed to process WeChat refund:', error);
      return {
        success: false,
        refundId: '',
        amount: request.amount || 0,
