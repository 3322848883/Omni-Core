...
export class SettlementService {
   ...
   * 计算支付手续费
   */
  private calculateFee(provider: string, amount: number): number {
    // 根据不同支付渠道设置不同的手续费率
   ...
   * 创建结算报表
   */
  async createSettlement(request: CreateSettlementRequest): Promise<SettlementReport> {
    try {
      const reportNo = this.generateReportNo();

      const [settlement] = await db('settlement_reports').insert({
        report_no: reportNo,
      ...
      logger.info(`Created settlement report: ${reportNo} for provider ${request.provider}`);
      return settlement;
    } catch (error) {
      logger.error('Failed to create settlement report:', error);
      throw error;
    }
  }
   ...
   * 执行结算
   */
  async processSettlement(settlementId: number): Promise<SettlementReport> {
    try {
      // 开始事务
      ...
      try {
        ...
        logger.info(`Processed settlement report: ${updatedSettlement.report_no} with ${orders.length} orders`);
      ...
      logger.error(`Failed to process settlement report ${settlementId}:`, error);
      throw error;
    }
  }
  ...
  async getSettlementReports(params: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ items: SettlementReport[]; pagination: any }> {
    try {
      const page = params.page || 1;
      const limit = params.limit || 20;
      const offset = (page - 1) * limit;

      let query = db('settlement_reports');
      ...
      logger.error('Failed to get settlement reports:', error);
      throw error;
    }
  }
   ...
   * 获取结算详情
   */
  async getSettlementDetails(settlementId: number, params: {
    page?: number;
    limit?: number;
  }): Promise<{ items: SettlementDetail[]; pagination: any }> {
    try {
      const page = params.page || 1;
      const limit = params.limit || 20;
      const offset = (page - 1) * limit;

      const query = db('settlement_details').where('settlement_report_id', settlementId);
      ...
      logger.error(`Failed to get settlement details for ${settlementId}:`, error);
      throw error;
    }
  }
   ...
   * 获取结算报表详情
   */
  async getSettlementById(id: number): Promise<SettlementReport | null> {
    try {
      const settlement = await db('settlement_reports').where('id', id).first();
      return settlement;
    } catch (error) {
      logger.error(`Failed to get settlement report ${id}:`, error);
      throw error;
    }
  }
}
