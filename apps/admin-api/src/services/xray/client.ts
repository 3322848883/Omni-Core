...
export class XrayClient {
  ...
  constructor(apiHost?: string, apiPort?: number) {
    this.apiHost = apiHost || config.xray.host || 'localhost';
    this.apiPort = apiPort || config.xray.port || 10085;
    ...
    this.client.interceptors.request.use(
      (config) => {
        logger.debug(`Xray API Request: ${config.method?.toUpperCase()} ${config.url}`);
        return config;
      },
  ...
  private handleError(error: AxiosError): void {
    if (error.code === 'ECONNREFUSED') {
      logger.debug(`Xray API connection refused at ${this.apiHost}:${this.apiPort}`);
      this.isConnected = false;
    } else if (error.code === 'ETIMEDOUT') {
      logger.debug('Xray API request timeout');
    } else if (error.response) {
      logger.debug(`Xray API error: ${error.response.status}`);
    } else {
      logger.debug('Xray API error:', error.message);
    }
  }
  ...
  // 查询统计信息
  async queryStats(pattern: string, reset: boolean = false): Promise<XrayStatsResponse> {
    try {
      ...
      logger.debug(`Failed to query stats with pattern ${pattern}`);
      return { stat: [] };
    }
  }

  // 获取系统统计
  async getSysStats(): Promise<XraySysStatsResponse> {
    try {
  ...
  // 获取所有入站统计
  async getAllInboundStats(reset: boolean = false): Promise<Map<string, TrafficStats>> {
    try {
      ...
      logger.debug('Failed to get all inbound stats');
      return new Map();
    }
  }

  // 获取用户统计
  async getUserStats(email: string, reset: boolean = false): Promise<TrafficStats> {
    try {
  ...
  // 获取所有用户统计
  async getAllUserStats(reset: boolean = false): Promise<Map<string, TrafficStats>> {
    try {
  ...
  // 添加入站
  async addInbound(inboundConfig: XrayInboundConfig): Promise<boolean> {
    try {
      await this.client.post('/handler/addInbound', inboundConfig);
      return true;
    } catch (error) {
      logger.debug('Failed to add inbound');
      return false;
    }
  }
  ...
  // 添加用户
  async addUser(inboundTag: string, user: any): Promise<boolean> {
    try {
      ...
      logger.debug(`Failed to add user to inbound ${inboundTag}`);
      return false;
    }
  }
  ...
  async removeUser(inboundTag: string, email: string): Promise<boolean> {
    try {
      ...
      logger.debug(`Failed to remove user ${email} from inbound ${inboundTag}`);
      return false;
    }
  }
  ...
  // 重启日志
  async restartLogger(): Promise<boolean> {
    try {
      await this.client.post('/logger/restart');
      return true;
    } catch (error) {
      logger.debug('Failed to restart logger');
      return false;
    }
  }
  ...
  async getNodeStatus(): Promise<any> {
    try {
      ...
      logger.debug('Failed to get node status');
      return {
        connected: false,
        sysStats: null,
        inboundStats: {},
  ...
  // 获取入站列表
  async getInbounds(): Promise<XrayInboundConfig[]> {
  ...
  async hotReloadConfig(config: { inbounds: XrayInboundConfig[] }): Promise<boolean> {
    ...
    logger.debug('Hot reload not supported');
    return false;
  }
}

// 全局 Xray 客户端实例
let xrayClientInstance: XrayClient | null = null;

export function getXrayClient(apiHost?: string, apiPort?: number): XrayClient {
...
export function resetXrayClient(): void {
